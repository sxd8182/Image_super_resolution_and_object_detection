...
SOURCE_TRAIN_TXT = "/mnt/users/sdevkota/train_yolo_m.txt"
SOURCE_VAL_TXT = "/mnt/users/sdevkota/val_yolo_m.txt"
...
EPOCHS = 25
BATCH = 32
RESULT_CSV = "/mnt/users/sdevkota/output/upscale_experiments_256_to_640_using_SR/results_256_to_640_using_SR.csv"
...
for res in RES_LIST:
    print(f"\n [RES {res}] Downscaling  And Upscaling ")
    exp_dir = f"/mnt/users/sdevkota/output/upscale_experiments_256_to_640_using_SR/train_from_{res}_to_{BASE_RES}"
    ...
    for subset, txt_file in zip(['train', 'val', 'test'], [SOURCE_TRAIN_TXT, SOURCE_VAL_TXT, SOURCE_TEST_TXT]):
        ...
        for img_path in img_paths:
            ...
            try:
                hg_result = hg_pipeline(images = small_pil)
                if isinstance(hg_result, Image.Image):
                    upscaled = cv2.cvtColor(np.array(hg_result), cv2.COLOR_RGB2BGR)
                

            except Exception as e:
                print(f"Hugging face Super Resolution failed for {img_path}: {e}")
                continue
...
print(f"\n Experiment Concluded and all results saved to {RESULT_CSV} from experiment downscale upscale 256 to 640 using SR")

...
# importing Real-ESRGAN dependencies
...
EXP_DIR = "/mnt/users/sdevkota/output/original_with_RealESRGAN/train_original_to_640"
RESULT_CSV = "/mnt/users/sdevkota/output/original_with_RealESRGAN/results_real_esrgan_from_original.csv"
...
# Processing training and validation sets with Real-ESRGAN
for subset, txt_file in zip(['train', 'val'], [SOURCE_TRAIN_TXT, SOURCE_VAL_TXT]):
...
# Writing YOLO training data.yaml
...
# Training YOLO model on enhanced images
...
test_yaml = os.path.join(EXP_DIR, "data_test_real.yaml")
with open(test_yaml, "w") as f:
    f.write(f"path: {EXP_DIR}\n")
    f.write("train: images/train\n")
    f.write("val: images/val\n")
    f.write(f"test: {SOURCE_TEST_TXT}\n")
    f.write("names: ['Normal sperm', 'Sperm cluster', 'Small or pinhead sperm']\n")

print("Evaluating model on original test images (YOLO auto-pads)")
